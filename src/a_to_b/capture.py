...
class Capture:
    handle: c.c_void_p = c.c_void_p()
    buffer: npt.NDArray = np.zeros(shape=(0), dtype=np.uint8)
    buffer_internal: c.c_void_p = c.c_void_p()
    ...
    def __init__(self) -> None:
        ...
        self.buffer = np.full(shape=(height, width, 4), fill_value=[0, 0, 255, 255], dtype=np.uint8)
        self.buffer_internal = self.buffer.ctypes.data_as(c.c_void_p)
    ...
    def get_frame(self) -> npt.NDArray:
        ...
        Returns:
            npt.NDArray: 
            Internal buffer after frame capture update. BGRA format.
        Warning:
        ...
        """
        print(cptr_dll.capture_state_get_frame(self.handle, self.buffer_internal))
        return self.buffer
